  ...
  const data = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      daily.push({
        ...
        count: withStatus.filter((p) => p.visit_date.split("T")[0] === key)
          .length,
      });
    }
    ...
    withStatus.forEach((p) =>
      unitCounts.set(p.unit_number, (unitCounts.get(p.unit_number) ?? 0) + 1),
    );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {Object.keys(data.statusBreakdown).map((s) => {
              const pct =
                data.total > 0
